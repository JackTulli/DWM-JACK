...
tile(Monitor *m)
{
	...
	for (n = 0, c = nexttiled(m->clients); c; c = nexttiled(c->snext), n++);
	if (n == 0)
		return;
	...
	if (n > m->nmaster)
		...
		mw = m->ww - gappx;
	for (i = 0, my = ty = gappx, c = nexttiled(m->clients); c; c = nexttiled(c->snext), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - gappx;
			resize(c, m->wx + gappx, m->wy + my, mw - (2 * gappx), h, 0);
			if (my + h + gappx < m->wh)
				my += h + gappx;
		} else {
			h = (m->wh - ty) / (n - i) - gappx;
			resize(c, m->wx + mw + gappx, m->wy + ty, m->ww - mw - (2 * gappx), h, 0);
			if (ty + h + gappx < m->wh)
				ty += h + gappx;
		}
}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		/* Debug: Log monitor and bar geometry */
		fprintf(stderr, "Monitor %d: x=%d, y=%d, width=%d, height=%d, bar_x=%d, bar_y=%d, bar_width=%d, rotated=%s\n",
			m->num, m->wx, m->my, m->mw, m->mh, bar_x, bar_y, bar_width, m->mw < m->mh ? "yes" : "no");
		m->barwin = XCreateWindow(dpy, root, bar_x, bar_y, bar_width, bh, 0, DefaultDepth(dpy, screen),
...
{
	m->wy = m->my; /* Start at monitor top */
	m->wh = m->mh; /* Full monitor height */
	if (m->showbar) {
		if (m->topbar) {
